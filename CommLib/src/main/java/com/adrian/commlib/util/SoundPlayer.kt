 ...
 * Description:自定义基于SoundPool的简易播放器
 */
class SoundPlayer(val context: Context) {
    ...
    var onPlayCompleteListener: ((index: Int) -> Unit)? = null
    var onPlayExceptionListener: ((index: Int) -> Unit)? = null
    ...
    private fun getSoundDuration(@RawRes rawResId: Int) = MediaPlayer.create(context, rawResId).duration
    ...
    @Synchronized fun play(index: Int) {
        ...
        if (streamId != 0) {
            ...
            executor.schedule({
                isPlaying = false
                onPlayCompleteListener?.invoke(curIndex)
                              }, duration, TimeUnit.MILLISECONDS)
        } else {
            onPlayExceptionListener?.invoke(curIndex)
        }
    }
